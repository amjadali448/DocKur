...
import { geNetworkData, CreateNetwork, ImportImage, ContainerList, ListofImages, CreateImage } from '../Redux/Auth/action'//APi Redux function import
import "./header.css";
import axios from 'axios';


export const Home = ({ geNetworkData, Networklist, CreateNetwork, ImportImg, containerLists,
  ContainerList,
  ImportImage, listContainer, ListofImages
  ...
  const [importImg, setImportImg] = useState(ImportImg);
  ...
  useEffect(() => {
    ...
    geNetworkData()
    ImportImage()
    setImportImg(ImportImg)
    // setContainerList(containerLists)
  ...
  const convertHandler = () => {
    ...
    if (selectedSpecificFile) {
      CreateImage(selectedSpecificFile)
  ...
  const [eVolumeData, setEVolumeData] = useState([])
  ...
  const pathHandler = () => {
    ...
    setVolumeData(prevData => [...prevData, data]);
    console.log('data', data)
    debugger
    // console.log();
  }
  const VolumesHandler = () => {
    setVoluemShow(true)
    const data = {
      variable: variable,
      value: value
    }
    setEVolumeData(prevData => [...prevData, data]);
    console.log('data', data)
    debugger
    // console.log();
  }
  ...
  const handleDrop = (e, id) => {
    ...
    // const filteredData = containerLists.filter((item) => item.Id === cardText);
    ...
    if (draggedCards.length <= 11 && draggedCards.filter(x => x === cardText).length === 0) {
      ...
      setlistImgLog(cardText)
      //   const filteredData = containerLists.filter((item) =>
      //   item.Id === id
      // );
      // setContainerList(containerLists, ...filteredData)
    }
  };
  ...
  const handleSearch = (query) => {
    const filtered = ImportImg.filter((item) =>
  ...
  const [listImgLog, setlistImgLog] = useState();
  ...
  const handleOptionSelect = (option, id) => {
    ...
    if (option === 'Inspect' && Networklist) {
      ...
      const filteredData = Networklist.filter((item) =>
        item.Id === id
      );
      setNetworklist(filteredData)
    }
    if (option === 'Inspect' || option === 'logs' && containerList) {
      setIsDropdownOpen(false)
      const filteredData = containerLists.filter((item) =>
        item.Id === id
      );
    ...
    else if (option === 'Inspecte' || option === 'logse' && draggedCards) {
      setIsDropdownOpen(false)
      const filteredData = containerLists.filter((item) => item.Id === listImgLog);
      setDraggedCards(filteredData)
      setPopupCard(false)
      ...
      <div className="App flex justify-items-center items-center flex-col w-full   " >
          ...
          <div className=" popup  w-3/12 items-center ">
            {selectedFolder && selectedFiles || selectedSpecificFile ? (
              ...
              <div className='flex flex-col items-center'>
                ...
                <label className='text-teal-400 text-center items-center my-4' htmlFor="fileInput"> Upload the folder to access its contents</label>
                <input
                  type="file"
                  id="fileInput"
                  webkitdirectory=""
                ...
                <label htmlFor="specificFileInput" className='text-teal-400 text-center items-center my-4'> Select a specific file</label>
        ...
        {activePopup === 'listImgPopup' &&
          <div className="w-full  h-full">
            ...
            {popupCard &&
              <div id='pop1' className="p-2 popup-main  shadow rounded overflow-y-scroll scrollbar  scrollbar-thin  scrollbar-thumb-gray-900 scrollbar-track-gray-500 ">
                ...
                <div className='flex justify-between items-center  p-2'>
                  <button className='bg-gray-300 hover:bg-gray-200 px-2 w-16 '>All</button>
                  <button className='bg-gray-300 hover:bg-gray-200 px-2 w-16'>Used</button>
                  <button className='bg-gray-300 hover:bg-gray-200 px-2 w-16'>Unused</button>
                </div>
                {/* Popup cards */}
                {containerLists?.map((x, y) => (
                  ...
                  >
                    <div className='flex '>
                      ...
                      <div className='ml-3 py-2'>
                        <label className='text-white text-md font-semibold w-32 truncate'>{x.Names}</label>
                        <p className='text-teal-400 text-sm w-32 truncate'>{x.Command}t</p>
                      </div>
                    </div>
            ...
            {inspectPopup && selectedOption === 'logse' || selectedOption === 'Inspecte' ? (
              <div className='w-11/12 mt-5 m-auto'>

                {draggedCards.map((x, y) => (
                  <div className='w-11/12 mt-5 m-auto'>
                    ...
                    {logsInsSwitch ?
                      ...
                      <div className="  bg-white border border-gray-300 rounded-lg shadow-lg p-2 mt-2">
                        <div className=' text-left p-1 py-2 mx-2 flex  border-b-[2px]  border-b-gray-200   '>
                          ...
                          <p className='cursor-pointer text-sbase  text-gray-700 flex items-center'>1.25.222</p>
                        </div>
                        <div className=' text-left p-1 py-2 mx-2 flex  border-b-[2px]  border-b-gray-200  '>
                  ...
                  <div key={index} className="dragged-card md:w-2/6 md:h-40 sm:h-full sm:w-full  flex flex-col  justify-center items-center ">
                    ...
                    <div style={{ width: '270px' }} className=" h-4/6 w-4/6 m-1 card-bg-color  flex flex-col justify-center" id={"card_" + index}>
                      ...
                      <div className='flex justify-evenly px-8 items-center w-full'>
                        <svg onClick={() => setRunPopupVisible({ isopen: true, id: "card_" + index })} className="cursor-pointer" width="31" height="31" viewBox="0 0 31 31" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            {runPopupVisible.isopen && (
              <div className="popup w-2/6 h-2/3">
                <>
                  <svg className='ml-3' width="41" height="34" viewBox="0 0 41 34" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fillRule="evenodd" clipRule="evenodd" d="M19.5405 0.196731C20.1534 -0.0655785 20.8466 -0.0655768 21.4595 0.196735C24.5565 1.52222 27.6536 2.80552 30.7507 4.08882C33.6765 5.30113 36.6023 6.51345 39.528 7.76134C40.4218 8.14256 41 9.02386 41 9.99822V24.0018C41 24.9761 40.4218 25.8574 39.528 26.2387C36.6019 27.4868 33.6756 28.6993 30.7493 29.9118C27.6526 31.1949 24.556 32.478 21.4595 33.8033C20.8466 34.0656 20.1534 34.0656 19.5405 33.8033C16.4435 32.4778 13.3464 31.1945 10.2492 29.9111C7.32343 28.6988 4.39764 27.4865 1.47196 26.2387C0.578183 25.8574 0 24.9761 0 24.0018V9.99821C0 9.02385 0.578178 8.14256 1.47195 7.76134C4.39814 6.51323 7.32444 5.30071 10.2508 4.08818C13.3474 2.80509 16.444 1.522 19.5405 0.196731ZM36.4445 12.1841C36.4445 11.5132 36.9883 10.9693 37.6593 10.9693C38.3302 10.9693 38.8741 11.5132 38.8741 12.1841V23.4658C38.8741 24.1367 38.3302 24.6806 37.6593 24.6806C36.9883 24.6806 36.4445 24.1367 36.4445 23.4658V12.1841ZM33.1037 12.7974C32.4328 12.7974 31.8889 13.3413 31.8889 14.0122V25.2939C31.8889 25.9648 32.4328 26.5087 33.1037 26.5087C33.7747 26.5087 34.3185 25.9648 34.3185 25.2939V14.0122C34.3185 13.3413 33.7747 12.7974 33.1037 12.7974ZM27.3333 15.8404C27.3333 15.1695 27.8772 14.6256 28.5481 14.6256C29.2191 14.6256 29.7629 15.1695 29.7629 15.8404V27.1221C29.7629 27.793 29.2191 28.3369 28.5481 28.3369C27.8772 28.3369 27.3333 27.793 27.3333 27.1221V15.8404ZM22.4741 17.6685C22.4741 16.9976 23.018 16.4537 23.6889 16.4537C24.3598 16.4537 24.9037 16.9976 24.9037 17.6685V28.9502C24.9037 29.6211 24.3598 30.165 23.6889 30.165C23.018 30.165 22.4741 29.6211 22.4741 28.9502V17.6685Z" fill="#76CDB2" />
                  </svg>
                ...
                <div className=' '>
                  ...
                  {optionalData ?
                    <div className='overflow-y-scroll scrollbar  scrollbar-thin  scrollbar-thumb-gray-900 h-80'>
                      <input type='text' placeholder='Container Name' className={'bg-white-600 text-gray-200 w-4/5 mb-3 shadow border rounded rounded p-1 text-sm text-gray-700 focus:border-teal-500 focus:outline-none '} />
                      ...
                      <select name="assessingclinicianuserid" id="ddlClinican" className=" bg-white-600 text-gray-400 w-5/6 mb-3 shadow border rounded rounded p-1 text-sm  focus:border-teal-500 focus:outline-none  ">
                        <option className='text-gray-200'>~Select Network~</option>
                        {Networklist.map((x, y) => (
                          <option className="text-black-200 " value={x.Name}> {x.Name} </option>
                        ))}

                      </select>
                      <p className='text-gray-300  font-semibold '>Volumes</p>
                      <div className='w-full flex justify-center items-center '>
                        <input onChange={(e) => setHost(e.target.value)} type='text' placeholder="Host Path" className={'bg-white-600 text-gray-200 mr-2 w-3/6  shadow border rounded rounded p-1 text-sm text-gray-700 focus:border-teal-500 focus:outline-none '} />
                        <input onChange={(e) => setContainer(e.target.value)} type='text' placeholder="Container Path" className={'bg-white-600 text-gray-200 w-3/6 shadow border rounded rounded p-1 text-sm text-gray-700 focus:border-teal-500 focus:outline-none '} />
                        <button onClick={pathHandler} className=" text-white hover:bg-teal-400  ml-2 font-bold  rounded  justify-between flex items-center">
                      ...
                      {hostshow &&
                        <>
                          {volumeData && volumeData.map((x, y) => (
                            <div className='bg-white-800 w-full shadow border rounded rounded p-1  border-teal-500 my-1'>
                              <p type='text' placeholder="Container Path" className={' text-gray-300  text-sm '} >Host Path: {x?.host}</p>
                              <p type='text' placeholder="Container Path" className={' text-gray-300 text-sm  '} >Container Path: {x?.container}</p>
                            </div>
                          ))}
                        </>
                      }
                      ...
                      <div className='w-full flex justify-arround'>
                        ...
                        <button onClick={VolumesHandler} className=" text-white hover:bg-teal-400  ml-2 font-bold  rounded  justify-between flex items-center">
                          <svg width="15" height="14" viewBox="0 0 17 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <line x1="8.3291" y1="3.23891e-08" x2="8.3291" y2="16" stroke="white" stroke-width="1.5" />
                            <line x1="16.8423" y1="8.3291" x2="0.000179291" y2="8.3291" stroke="white" stroke-width="1.5" />
                          </svg>
                      ...
                      {/* add map here  */}
                      {voluemShow &&
                        <> {eVolumeData && eVolumeData.map((x, y) => (
                          <div className='bg-white-800 w-full shadow border rounded rounded p-1  border-teal-500 my-1'>
                            <p type='text' placeholder="Container Path" className={' text-gray-300  text-sm '} >Variable: {x?.Variable}</p>
                ...
                <div className='flex justify-around ' style={{bottom:'5%', position:'absolute', alignSelf:'center'}}>
                  <button onClick={() => RunHandler()} className="mt-8 w-20 bg-teal-400  font-bold py-1 px-4 rounded  items-center">
                    Run
                  </button>
                  <button style={{marginLeft:'5%'}} onClick={() => setRunPopupVisible({ isopen: false, id: "" })} className="mt-8 w-20 bg-gray-400  font-bold py-1 px-4 rounded  items-center">
                    Close
                  </button>
                </div>
              </div>
        ...
        {activePopup === 'containers' &&
          < div className='w-full flex flex-wrap overflow-y-scroll scrollbar  scrollbar-thin  scrollbar-thumb-gray-900 scrollbar-track-gray-500 justify-between  p-3 '>
            ...
            {containerList && selectedOption === 'logs' || selectedOption === 'Inspect' ? (
              <>
                {containerList?.map((x, y) => (
                  <div className='w-11/12 mt-5 m-auto'>
                    <div className='flex justify-center py-2 w-full justify-between'>
                      <div className='flex items-center'>
              ...
              :
              <>
                {containerLists.map((x, index) => (
                  <div key={index} className=" h-4/6 m-1 card-bg-color  flex flex-col justify-center" id={"card_" + index}>
                    <div className='flex justify-between items-center mx-3'>
                      ...
                      <div className="inline-block relative">
                        ...
                        >
                          ...
                          <li>
                            <button
                              onClick={() => handleOptionSelect('logs', x.Id)}
                              className="block px-4 py-2 bg-gray-300  hover:bg-gray-500 hover:text-white w-full text-left" > logs
                            </button>
                          </li>
                        </ul>
                    ...
                    <div className='flex justify-evenly px-8 items-center w-full'>
                      ...
                      <svg onClick={() => RunHandler} className="cursor-pointer" width="32" height="31" viewBox="0 0 32 31" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M30.421 15.25C30.421 23.3134 23.8844 29.85 15.821 29.85C7.75769 29.85 1.22104 23.3134 1.22104 15.25C1.22104 7.18664 7.75769 0.65 15.821 0.65C23.8844 0.65 30.421 7.18664 30.421 15.25Z" stroke="#76CDB2" strokeWidth="1.3" />
                        <rect x="11.1806" y="10.6108" width="9.28261" height="9.28261" fill="#C89125" />
                      </svg>
                      <svg className="cursor-pointer" width="31" height="31" viewBox="0 0 31 31" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className='flex flex-col justify-center items-center '>
              {importImg?.map((x, y) => (
                <div key={y} style={{ backgroundColor: '#17222A' }} className="flex w-11/12 items-center justify-between p-2 my-4 pointer-events">
                  ...
                  <button
                    onClick={pullHandler}
                    type="submit" className="text-white  right-2.5 bottom-2.5 current-pointer  hover:bg-gray-300 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 bg-gray-400  dark:focus:ring-gray-700">Pull Image</button>
                </div>
              ))}
            </div>
          </div>
            ...
            {popupCardNetwork &&
              <div style={{ width: '300px' }} id='pop1' className="p-2 absolute  popup-main  shadow rounded overflow-y-scroll scrollbar  scrollbar-thin  scrollbar-thumb-gray-900 scrollbar-track-gray-500 ">
                <div className='flex justify-between mb-2'>
                  <button className='bg-gray-300 rounded hover:bg-gray-200 px-2 w-9/12 '>Remove Unused Networks</button>
                ...
                {networklist.map((x, y) => (
                  < div key={y} >
                    ...
                    <div className="  bg-white border border-gray-300 rounded-lg shadow-lg p-2 mt-2">
                      <div className=' text-left p-1 py-2 mx-2 flex  border-b-[2px]  border-b-gray-200   '>
                        <p className='cursor-pointer text-sbase font-semibold w-52 text-gray-700'>Path</p>
                        <p className='cursor-pointer text-sbase  text-gray-700 flex items-center'>1.25.21</p>
                      </div>
                      <div className=' text-left p-1 py-2 mx-2 flex  border-b-[2px]  border-b-gray-200  '>
                        <p className='cursor-pointer text-sbase font-semibold w-52 text-gray-700'>NGINX_VERSION</p>
                        <p className='cursor-pointer text-sbase  text-gray-700 flex items-center'>342343</p>
...
const mapStateToProps = (state) => ({
  ...
  ImportImg: state.authReducer.importImg,
  containerLists: state.authReducer.containerLists,
  listContainer: state.authReducer.listContainer,//listContainer state name in reducer

  // call state here from recuder and in props Line (10)
})

const mapDispatchToProps = { geNetworkData, CreateNetwork, ImportImage, ContainerList, ListofImages, CreateImage }//  Api Call funtion form Action also imort function from actions & also call in props Line(10) && 
